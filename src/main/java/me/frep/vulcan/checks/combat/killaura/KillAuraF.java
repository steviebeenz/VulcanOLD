...
public class KillAuraF extends Check {
    ...
    public KillAuraF() {
        super("KillAuraF", "Kill Aura (Type F)", CheckType.COMBAT, true, false, 8);
    }
    ...
    public void onReceive(PacketReceiveEvent e) {
        ...
        if (e.getPacketId() == PacketType.Client.USE_ENTITY) {
            ...
            if (data.killAuraFInventoryOpen) flag(p, null);
        }
        if (e.getPacketId() == PacketType.Client.FLYING) {
            data.killAuraFInventoryOpen = false;
        }
        if (e.getPacketId() == PacketType.Client.CLIENT_COMMAND) {
            data.killAuraFInventoryOpen = true;
        }
    }
}